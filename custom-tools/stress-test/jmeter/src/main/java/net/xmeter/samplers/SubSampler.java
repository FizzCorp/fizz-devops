...
public class SubSampler extends AbstractMQTTSampler {
	...
	public SampleResult sample(Entry arg0) {
		...
		setListener(sampleByTime, sampleCount, vars);
		...
		if (topics == null) {
			...
			listenToTopics(vars, topicsName);  // TODO: run once or multiple times ?
		} else {
	...
	private void listenToTopics(final JMeterVariables vars, final String topicsName) {
		boolean hasSubscribed = Objects.nonNull(vars.getObject("has_subscribed"));
		if (hasSubscribed) {
			return;
		}
		...
		connection.subscribe(topics, new Callback<byte[]>() {
			...
			public void onSuccess(byte[] value) {
				...
				vars.putObject("has_subscribed", true);
			}
	...
	private void setListener(final boolean sampleByTime, final int sampleCount, final JMeterVariables vars) {
		connection.listener(new Listener() {
			...
			@Override
			public void onFailure(Throwable value) {
                logger.severe("[FizzTest] Sub on Failure");
            }
			...
			public void onConnected() {
                logger.severe("[FizzTest] Sub on Connected");
			}
		});
	}
